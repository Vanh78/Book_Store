...
public class DonHangDAO implements DAOInterface<DonHang> {
		...
		try {
			...
			while (rs.next()) {
				...
				DonHang dh = new DonHang(maDH, khachHang, diaChiNguoiMua, diaChiNhanHang, trangThai, hinhThucThanhToan,
						hinhThucThanhToan, soTienDaThanhToan, soTienConThieu, ngayDatHang, ngayGiaoHang);
	...
	public int insert(DonHang t) {
		...
		String sql = "INSERT INTO donhang(madonhang, khachhang, diachinguoimua, diachinguoinhan, trangthai, thanhtoan,tienthanhtoan, tienthieu,ngaydathang,ngaygiaohang)"
				+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		try {
			PreparedStatement st = ((Connection) con).prepareStatement(sql);
			st.setString(1, t.getMaDonhangString());
			...
			st.setDouble(7, t.getTienThanhToanString());
			st.setDouble(8, t.getTienThieuString());
	...
	public int update(DonHang t) {		
		...
		try {
			...
			st.setDouble(7, t.getTienThieuString());
